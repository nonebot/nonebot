...
你可能注意到了“天气”和“南京”之间有一个空格，这可能不太符合汉语的习惯，
但我们可以向 `@on_command` 中添加参数 `no_space=True`，这样 nonebot 就可以理解一个不带空格的指令。
**注意**：如果大量 `@on_command` 使用了 `no_space` 的话，请尽量保持命令的长度在一个较小的集合中，以便获得更好的性能。
（例如：将所有指令统一为两个汉字，这样只需要一次字典查询就能找到所需的指令）
:::
